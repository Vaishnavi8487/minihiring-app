...
export const jobsApi = {
  ...
  async reorderJob(fromOrder: number, toOrder: number) {
    const res = await fetch('/api/jobs/dummy-id/reorder', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fromOrder, toOrder }) });
    await json(res);
  },
...
export const candidatesApi = {
  ...
  async updateCandidate(id: string, updates: Partial<Candidate>) {
    const res = await fetch(`/api/candidates/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updates) });
    return json(res);
  },
  ...
  async addNote(candidateId: string, note: string) {
    const res = await fetch(`/api/candidates/${candidateId}/notes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ note }) });
    await json(res);
  }
};
...
export const assessmentsApi = {
  ...
  async saveAssessment(jobId: string, sections: any[]) {
    const res = await fetch('/api/assessments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ job_id: jobId, sections }) });
    return json(res);
  },

  async submitResponse(assessmentId: string, candidateId: string, responses: Record<string, any>) {
    const res = await fetch('/api/assessment_responses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assessment_id: assessmentId, candidate_id: candidateId, responses }) });
    return json(res);
  },
  ...
  async getResponse(assessmentId: string, candidateId: string) {
    ...
    const res = await fetch(`/api/assessment_responses?${qs.toString()}`);
    return json(res);
  }
};
